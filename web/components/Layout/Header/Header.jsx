...
export default function Header() {
  ...
  // Fixed: Save both product name and ID for better functionality
  const addToRecentSearches = (searchItem) => {
    if (!searchItem) return;

    // Handle both string (search queries) and object (product selections)
    const item =
    ...
    try {
      ...
      setRecentSearches(updated); // Update state immediately
    } catch (err) {
      console.error("Local storage error:", err);
    }
  };

  // Fixed: Load recent searches on mount
  useEffect(() => {
    try {
  ...
  // Fixed: Debounce function with proper cleanup
  ...
  // Fixed: Search execution with navigation
  ...
  // Fixed: Backend search save with better error handling
  ...
  // Fixed: Product click handler - save complete product info
  const handleProductClick = (product) => {
    ...
    // Save complete product info to recent searches
  ...
  // Fixed: Recent search/product click handler
  ...
  const handleCartClick = (e) => {
    if (!isLoggedIn) {
      ...
      setTimeout(() => setShowCartPrompt(false), 3000); // Auto-hide after 3 seconds
    }
  };
